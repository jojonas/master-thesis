...
In this section, we want to regard the special cases of a normal probability distribution and a log-normal distribution and derive a formula for adapting the distribution width parameter $\sigma$ such that the $p$-value to include \Ntrue in a (log-)normal distribution parametrized around \Nmc will remain the same as for \Nmc parametrized by \Ntrue. However, some of the ideas mentioned here can be adapted to other distributions too. 
...
On the right hand side, we have the distribution used for calculating the $p$-value:
\begin{equation}
...
For this purpose, we first derive a general solution for $\Phi(x)$, which is the cumulative distribution function of a standard normal distribution $\mathcal N(\mu=0, \sigma=1)$:
...
Here, we have used that $\Phi$ can be expressed in terms of the error function, which in turn can be further expressed by an integral.
\begin{align}
...
We can now use the expression for $\Phi$ derived earlier to solve our initial equation:
\begin{align}
